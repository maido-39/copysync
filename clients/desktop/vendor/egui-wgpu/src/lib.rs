...
impl RenderState {
    ...
    pub async fn create(
        ...
        let adapter = {
            ...
            // CopySync patch: try a real (hardware) adapter first; if none exists
            // (GPU-less / RDP machines), retry requesting the SOFTWARE FALLBACK
            // adapter (WARP on Windows / llvmpipe) so a context can still be made
            // instead of failing with NoSuitableAdapterFound. A clipboard utility
            // must run on machines with no GPU at all.
            let mut adapter = instance
                ...
                .await;
            if adapter.is_none() {
                log::info!("no hardware wgpu adapter; retrying with the software fallback adapter (WARP/llvmpipe)");
                adapter = instance
                    .request_adapter(&wgpu::RequestAdapterOptions {
                        power_preference: config.power_preference,
                        compatible_surface: Some(surface),
